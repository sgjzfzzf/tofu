    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        if self.loss_type == "grad_ascent":
            ...
            attention_mask = torch.cat(
                (idk_attention_mask, retain_attention_mask), dim=0
            )
        ...
        with torch.no_grad():
            ...
            for eval_task in eval_cfg.eval_task:
                ...
                if world_size > 1:
                    if self.accelerator.is_local_main_process:
                        eval_logs = json.load(
                            open(os.path.join(curr_save_dir, f"{eval_task}_0.json"))
                        )
                        for i in range(1, world_size):
                            filename = os.path.join(
                                curr_save_dir, f"{eval_task}_{i}.json"
                            )
                            eval_logs = merge_dicts(
                                eval_logs, json.load(open(filename))
                            )

                        aggregated_eval_logs[f"{eval_task}.json"] = eval_logs

                        new_save_filename = os.path.join(
                            curr_save_dir, f"{eval_task}.json"
                        )
            ...
            if self.accelerator.is_local_main_process:
                ...
                aggregated_eval_log_filename = os.path.join(
                    curr_save_dir, "eval_log_aggregated.json"
                )
                ...
                if eval_cfg.retain_result is not None:
                    ...
                    forget_quality = get_forget_quality(
                        aggregated_eval_logs, retain_result
                    )
                    aggregate_stat = {**model_utility, **forget_quality}

                    # save aggregate_stat as csv
                    with open(
                        os.path.join(curr_save_dir, "aggregate_stat.csv"), "w"
                    ) as csvfile:
...
def custom_data_collator_forget(samples):
    forget_samples, retain_samples = [sample[0] for sample in samples], [
