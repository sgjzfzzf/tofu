...
def get_model_utility(eval_result_dict):
    ...
    for k, v in eval_result_dict.items():
        ...
        if "eval_log" in k:
            gt_probs = np.exp(
                -1 * np.array(list(eval_result_dict[k]["avg_gt_loss"].values()))
            )
            ...
            avg_false_prob = np.exp(
                -1
                * np.array(list(eval_result_dict[k]["average_perturb_loss"].values()))
            )
        ...
        for data_idx in data_indices:
            avg_paraphrase_np_values.append(
                eval_result_dict[k]["avg_paraphrased_loss"][data_idx]
            )
            avg_perturbed_np_values.append(
                eval_result_dict[k]["average_perturb_loss"][data_idx]
            )
        avg_paraphrase_np_values = np.exp(-1 * np.array(avg_paraphrase_np_values))
        avg_perturbed_np_values = np.exp(-1 * np.array(avg_perturbed_np_values)).mean(
            -1
        )
...
def get_forget_quality(unlearn_result, retain_result):
    ...
    unlearn_paraphrase_np_values = np.array(
        list(unlearn_forget_result["avg_paraphrased_loss"].values())
    )
    unlearn_perturbed_np_values = np.array(
        list(unlearn_forget_result["average_perturb_loss"].values())
    )
    unlearn_perturbed_np_values = unlearn_perturbed_np_values.mean(axis=-1)

    retain_paraphrase_np_values = np.array(
        list(retain_forget_result["avg_paraphrased_loss"].values())
    )
    retain_perturbed_np_values = np.array(
        list(retain_forget_result["average_perturb_loss"].values())
    )
